...
export type TargetVia =
	...
	| 'phone_not_member' // número completo que não está em nenhum grupo do escopo
...
export interface ResolvedTarget {
	jid: string | null // normalizado; null quando só temos o telefone ou nada casou
	phone: string | null // só dígitos
	foundIn: string[] // JIDs dos grupos do escopo onde o alvo está (vazio quando nada casou)
	via: TargetVia
...
function describe(rawJid: string, via: TargetVia, view: CommunityView, reason: string | null): ResolvedTarget {
	...
	return {
		...
		// participante sem phone_number na metadata) — a remoção funciona igual, casa pelo jid.
...
export function resolveTarget(msg: CmdMessage, cmd: ParsedCommand, view: CommunityView): ResolvedTarget | null {
	...
	// Número parcial: o erro mais comum é esquecer o DDI. Em vez de recusar de cara, procura
	// quem TERMINA com o que foi digitado.
	if (phone.length < FULL_PHONE_MIN) {
		...
		// nada casou e o número está incompleto — quase sempre erro de digitação.
		return { jid: null, phone, foundIn: [], via: 'phone_incomplete', reason }
	}

	// número COMPLETO que não está em nenhum grupo do escopo: não há de onde remover ninguém.
	return { jid: null, phone, foundIn: [], via: 'phone_not_member', reason }
}
