...
export function useSocket() {
	...
	const coreRef = useRef<ReturnType<typeof startCollectorCore> | null>(null)
	...
	useEffect(() => {
		...
		// trabalho SÓ-da-TUI por evento — storeMessage + dedup + parse + decrypt inline de enquete.
		async function handleMessagesUpsert(upsert: { type: string; messages: Array<{ key: WAMessageKey; message?: proto.IMessage | null; pushName?: string; messageTimestamp?: number | null }> }) {
		...
		function handleChatsUpdate(chatUpdates: Array<Record<string, unknown>>) {
			setChats((prev) => {
		...
		// metadata sintética emitida pelo núcleo (groupFetchAllParticipating) — atualiza nome/groupInfo + cache.
		function handleGroupMetadata(meta: GroupMetadata) {
			...
			setChats((prev) =>
				prev.map((c) =>
					c.jid === meta.id ? { ...c, name: info.subject, groupInfo: info } : c,
				),
			)
		}
		...
		const webhook = webhookUrl && webhookSecret ? { url: webhookUrl, secret: webhookSecret } : null

		const handle = startCollectorCore({
			authDir: 'baileys_auth_info',
			outboxPath: path.resolve('outbox.db'),
			// a TUI reusa o logger de arquivo existente como logger da app E do baileys (preserva o log atual).
			logger,
			...
			onEvent: (eventName, data) => {
				// feed de debug: todo evento entra (inclui o sintético groups.metadata).
				setDebugEvents((prev) => {
					const entry: DebugEvent = { timestamp: new Date(), event: eventName, summary: summarizeEvent(eventName, data) }
					const next = [...prev, entry]
					return next.length > MAX_DEBUG_EVENTS ? next.slice(-MAX_DEBUG_EVENTS) : next
				})

				if (eventName === 'messages.upsert') {
					void handleMessagesUpsert(data as Parameters<typeof handleMessagesUpsert>[0])
				} else if (eventName === 'chats.update') {
					handleChatsUpdate(data as Array<Record<string, unknown>>)
				} else if (eventName === 'groups.metadata') {
					handleGroupMetadata(data as GroupMetadata)
				}
			},
		})
	...
	const sendMessage = async (jid: string, text: string) => {
		await coreRef.current?.sendMessage(jid, text)
	}
